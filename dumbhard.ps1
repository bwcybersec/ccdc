...
Function Set_Internal_IPs_V {
    ...
    Start-Sleep -s 1
    $global:Docker = "10.7.2.2"
    $global:DNSNTP = "10.7.2.4"
    $global:Ubuntu18Web = "10.7.2.10"
    $global:UbuntuWrk = "10.7.1.10"
    $global:ADDNS = "10.7.1.40"
    $global:Splunk = "10.7.1.60"
    $global:EComm = "10.7.1.30"
    $global:WebMail = "10.7.1.70"
    $global:Windows2 = "10.7.1.80"
    
    $global:Internal = @($Docker,$DNSNTP,$Ubuntu18Web,$UbuntuWrk,$ADDNS,$Splunk,$EComm,$WebMail,$Windows2)
    cls
...
Function Win10 {
    cls
    Write-Host "Starting Function: Windows Lockdown" -ForegroundColor Cyan

    Start-Sleep -s 1
    netsh advfirewall firewall add rule name="WinRM"  dir=in action=allow enable=yes profile=any localport=5985 remoteip=any protocol=tcp  | Out-Null
    netsh advfirewall firewall add rule name="WinRM" dir=out action=allow enable=yes profile=any remoteport=5985 remoteip=any protocol=tcp  | Out-Null
    netsh advfirewall firewall add rule name="WinRM2"  dir=in action=allow enable=yes profile=any localport=5986 remoteip=any protocol=tcp  | Out-Null
    netsh advfirewall firewall add rule name="WinRM2" dir=out action=allow enable=yes profile=any remoteport=5986 remoteip=any protocol=tcp  | Out-Null
